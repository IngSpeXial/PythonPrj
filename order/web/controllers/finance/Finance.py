...
def index():
    ...
    if "status" in req and int(req["status"]) > -9 and int(req["status"]) != -1:
...
def set():
    ...
    # stat_info = db.session.query(PayOrder, func.sum(PayOrder.total_price).label("total")).filter(PayOrder.status==1).first()
    # app.logger.info(stat_info)

    total_money = 0
    for item in query:
        total_money = item.total_price + total_money

    resp_data["list"] = list
    resp_data["pages"] = pages
    # resp_data["total_money"] = stat_info[1] if stat_info[1] else 0.00
    resp_data["total_money"] = total_money
