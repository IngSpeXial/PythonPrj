...
def uploadPic():
    ...
    call_target = "window.parent.upload"
    if upfile is None:
        return "<script type='text/javascript'>{0}.error('{1}')</script>".format(call_target, "文件上传失败！")

    ret = UploadService.uploadByFile(upfile)
    if ret["code"] != 200:
        return "<script type='text/javascript'>{0}.error('{1}')</script>".format(call_target, "文件上传失败：" + ret["msg"])

    return "<script type='text/javascript'>{0}.success('{1}')</script>".format(call_target, ret["data"]["file_key"])
